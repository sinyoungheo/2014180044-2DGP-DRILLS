...
class Zombie:
    ...
    def build_behavior_tree(self):
        ...
        runaway_node = SelectorNode("Run of Move")
        runaway_node.add_children(run_from_player_node, move_to_player_node)

        chase_node = SequenceNode("Chase")
        chase_node.add_children(find_player_node, runaway_node)

        find_ball_node = LeafNode("Find ball", self.find_ball)
        move_to_ball = LeafNode("Move to ball", self.move_to_ball)
        colli_ball_node = SequenceNode("Colli ball")
        colli_ball_node.add_children(find_ball_node, move_to_ball)

        wander_eat_ball_chase_node = SelectorNode("WanderEatballChase")
        wander_eat_ball_chase_node.add_children(chase_node, colli_ball_node, wander_node)
    ...
    def draw(self):
        self.font.draw(self.x - 40, self.y + 60, 'HP : ' + str(self.hp), (255, 0, 0))
